...
- ✅ **#8 Difficulty knob** — `Location.difficulty` (low/mid/high) was UI-only; now actually scales gameplay. New `DIFFICULTY_MULTIPLIER` in `engine/map.ts` (low: 0.7, mid: 1.0, high: 1.4) drives both `BLOCKED_TILE_RATIO` and `THREAT_TILE_RATIO`. Existing per-location `blockedTileRatio` override still wins when set explicitly.
